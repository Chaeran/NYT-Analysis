function initKeywordGraph(containerSelector) {
    // Main SVG group with margin convention

    Promise.all([d3.json("data/nyt_keyword_graph.json"), d3.json("data/nyt_keyword_timeseries.json")])
...
const plotNodeLink = function (nodes, links, timeseriesData, sectionColor, lengthColor, cntSel) {

    const simWidth = 15000;
    const simHeight = 10000;

    const svg = d3.select(cntSel)
        .append("svg")
    ...
    const node = container.append("g")
        ...
        .on("click", (event, d) => {
            ...
            currKeyword = keyword;
            window.selectedKeyword = currKeyword;
