...
function initTreemap(containerSelector) {
  ...
  const plotTreeMap = function (data) {
    ...
    d3.treemap().tile(d3.treemapSquarify).size([width, height]).padding(0)(
      // remove padding as the layout becomes weird with the smaller sections and later color in the nodes themselves
      root
    );
    ...
    function render(group, root) {
      ...
      const node = group.selectAll("g").data(nodes).join("g");
      ...
      node
        ...
        .on("click", (event, d) => {
          if (
            d.data.name !== "All Sections" &&
            !d.data.name.includes("Others")
          ) {
            window.state.selectedSection = d.data.name;
  ...
  window.updateTreemap = function (state) {
    if (!state.selectedSection || !globalRoot) return;

    const svg = d3.select("#treemapArea svg");
    svg
      .selectAll("rect")
      .transition()
      .duration(200)
    ...
    function findNode(node, targetName) {
      if (node.data.name === targetName) return node;
      if (node.children) {
    ...
    const targetNode = findNode(globalRoot, state.selectedSection);
    if (targetNode && globalZoomFunction) {
      svg
        .selectAll("rect")
        .filter(function (d) {
          return d.data.name === state.selectedSection;
        })
        .transition()
        .duration(200)
        .attr("stroke-width", 4)
