...
function initBurst(containerSelector){
  ...
  d3.csv("data/keyword_year_count_with_norm.csv").then(data => {
    ...
    select.on("change", event => {
      ...
      // shared state update (Burst -> other views)
      window.state.selectedKeyword = keyword;
      window.updateAll();
    ...
    d3.selectAll("input[name='mode']").on("change", event => {
      ...
      // optional: notify others state is still same keyword
      window.updateAll();
    ...
    window.updateBurst = function(state) {
      ...
      // Sync dropdown UI (if exists)
      d3.select("#keyword-select").property("value", kw);
